...
1. **A version id is not its content.** `tax_kb.tax_rule_version` rows are immutable by TKMS policy, but the artifacts that determine the *computed* result hang off shared tables that are **not** provably immutable: `rules.calc_formula.expression`, `rules.calc_formula_input`, `rules.rule_condition[_group]`, `rules.rule_outcome`, and `rules.calc_constant` (reference data, not version-scoped — keyed `(code, tax_year)` when this was written, and since [reference-data effective periods](reference-data-effective-periods.md) by `(code, tax_year, effective period)`). Editing a shared formula changes historical replay silently.
