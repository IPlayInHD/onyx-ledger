...
| **PD-10** | Object-store encryption, versioning, TLS and backup config absent from the repository | `DEPLOYMENT_CONFIGURATION_REQUIRED` | **VERSIONED ERASURE FIXED in B2A** — `ObjectStorage.hard_erase` enumerates every version and delete marker for one key, removes them by version id, and re-lists to confirm; a delete marker is never treated as erasure. Encryption, TLS and backup configuration remain deployment configuration and are NOT closed by this. |
...
| **PD-14** | `server/` (Node/Netlify Blobs) is a second application storing emails, bcrypt hashes, profiles and documents, and was absent from the first inventory pass | `CLOSED` (production-readiness entry) | **MEDIUM — CLOSED by decommissioning.** The tree is archived at `legacy/server/` and is no longer reachable by any deploy path: the root `netlify.toml` now builds only the certified frontend, and `backend/tests/security/test_legacy_engine_not_deployable.py` fails if any deployment configuration references it again (proven non-vacuous by reintroducing `base = "server"`). **Residual, and NOT closed by this entry:** whether a Netlify site was ever connected to this repository, and whether a Blobs store therefore holds real customer records, is an operational fact not observable from the source tree. If one exists it must still be reconciled and purged — decommissioning the code path does not delete data already written. Tracked as `OPERATIONAL_REVIEW_REQUIRED` under 11B9. |
| **PD-15** | The audit log's ownership key is not `actor_id`: registration and login run anonymously, so the rows creating an account and its credential record no actor and carry the identity in their payload. A de-identification phase keyed on `actor_id` alone would miss them. | `CLOSED` (11B6J) | Subject indirection shipped in 11B6/11B6B; the residual `actor_id` was measured after terminal removal and classified `ORPHANED_PSEUDONYMOUS_HISTORICAL_SECURITY_CORRELATOR`. See `docs/privacy/11b6j-terminal-account-removal.md`. |
| **PD-16** | The application runtime role is a member of the freshness capability role (`GRANT onyx_freshness_worker TO onyx_app_rw`, `29_ioe_outbox_and_projection.sql:517`), so any HTTP request-path session can `SET ROLE onyx_freshness_worker` and invoke the privileged cross-tenant outbox keyholes. Proven from a genuine runtime login (`session_user = onyx_test`, a member of `onyx_app_rw`): the role was assumed and `ioe.claim_freshness_events` claimed a real event. The keyhole architecture is sound; the membership defeats it. | `PRIVILEGE_ESCALATION_NOW` | **HIGH — found by Entry 11B5E, FIXED in 11B5E6C**. Dedicated LOGIN principal per privileged worker (`onyx_privacy_runtime`, `onyx_freshness_runtime`), each holding only its own capability and not `onyx_app_rw`; migration 0053 revokes the membership. Closure proof from a genuine runtime login: `SET ROLE onyx_freshness_worker` now returns *permission denied to set role*, where before it succeeded and claimed a real event. The worker keeps EXECUTE and still has NO direct queue-table access. |
...
| ~~`server/` (Node/Express)~~ | ~~**Netlify Blobs**~~ | **DECOMMISSIONED.** Archived to `legacy/server/`; the root `netlify.toml` now builds the certified frontend and nothing else. See PD-14. |
