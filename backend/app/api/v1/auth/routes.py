...
async def register(
    ...
    """
    # THE PROSE BELOW IS A COMMENT, NOT THE DOCSTRING, and the distinction is
    # load-bearing: FastAPI publishes a route's docstring as the endpoint's
    # OpenAPI `description`, which is API documentation a client reads. Why an
    # implementation decision was made belongs next to the code; what the
    # endpoint promises belongs above.
    #
    # THE FIRST VERIFICATION LINK IS SENT HERE because the account is unusable
    # until somebody opens it. Leaving the first send to the resend endpoint
    # looked tidier and shipped a product whose verification screen said "we
    # sent a link" over a link nobody had sent — caught by the browser suite,
    ...
    # caller can make Onyx mail somebody; here the caller is creating the
    # mailbox relationship, one message per account created, already bounded by
    # the registration throttle. Charging both would mean an address that has
